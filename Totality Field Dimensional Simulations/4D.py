...
# === Initialization ===
def initialize_field(D, N=32, L=10.0, seed=42):
    np.random.seed(seed)
    dx = L / N
...
# === 4D Laplacian update (8-neighbor) ===
def laplacian_4d(u, dx):
    lap = np.zeros_like(u)
    lap += np.roll(u, 1, axis=0)
    lap += np.roll(u, -1, axis=0)
    lap += np.roll(u, 1, axis=1)
    lap += np.roll(u, -1, axis=1)
    lap += np.roll(u, 1, axis=2)
    lap += np.roll(u, -1, axis=2)
    lap += np.roll(u, 1, axis=3)
    lap += np.roll(u, -1, axis=3)
    lap -= 8 * u
    return lap / dx**2
...
# === Main simulation ===
def simulate_D4(steps=500, dt=0.01, lambda_nl=0.57):
    u, dx = initialize_field(4, N=32)
    u_old = u.copy()
    peak_history = []
    snapshots = []

    for _ in tqdm(range(steps), desc="Simulating D=4"):
...
# === Structure analysis ===
def analyze_d4_structures(u, threshold_ratio=0.3, sigma=0.5):
    u_smooth = gaussian_filter(u, sigma=sigma)
    threshold = threshold_ratio * np.max(u_smooth)
    binary = u_smooth > threshold
...
if __name__ == "__main__":
    print("Simulating D=4...")
    peak_series, snapshots = simulate_D4()

    print("Analyzing final structure...")
    volumes, energies, box_sizes, fractals = analyze_d4_structures(snapshots[-1])
    ...
    if len(valid_fractals) > 0:
        ...
        plt.title("D=4 Fractal Dimension Estimates")
        plt.xlabel("Estimated D_f = log(V)/log(BoxSize)")
    else:
        plt.title("No valid fractal dimension")

    plt.tight_layout()
    plt.savefig("D4_structure_analysis.png")
    plt.show()
