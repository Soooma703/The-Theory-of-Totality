import numpy as np
import matplotlib.pyplot as plt
from scipy.ndimage import gaussian_filter, label, find_objects
from tqdm import tqdm


# === Initialization ===
def initialize_field(N=128, L=10.0, seed=42):
    np.random.seed(seed)
    dx = L / N
    x = np.linspace(0, L, N, endpoint=False)
    y = np.linspace(0, L, N, endpoint=False)
    z = np.linspace(0, L, N, endpoint=False)
    X, Y, Z = np.meshgrid(x, y, z, indexing="ij")
    u = np.zeros_like(X)
    for _ in range(10):
        cx, cy, cz = np.random.uniform(0, L, 3)
        w = np.random.uniform(0.1 * L, 0.3 * L)
        A = np.random.uniform(0.5, 1.0)
        u += A * np.exp(-((X - cx) ** 2 + (Y - cy) ** 2 + (Z - cz) ** 2) / (2 * w**2))
    return u, dx


# === Time evolution ===
def update_field(u, u_old, dx, dt, lambda_nl=0.57, c=1.0):
    u_new = np.zeros_like(u)
    u_new[1:-1, 1:-1, 1:-1] = (
        2 * u[1:-1, 1:-1, 1:-1]
        - u_old[1:-1, 1:-1, 1:-1]
        + dt**2
        * (
            c**2
            * (
                u[2:, 1:-1, 1:-1]
                + u[:-2, 1:-1, 1:-1]
                + u[1:-1, 2:, 1:-1]
                + u[1:-1, :-2, 1:-1]
                + u[1:-1, 1:-1, 2:]
                + u[1:-1, 1:-1, :-2]
                - 6 * u[1:-1, 1:-1, 1:-1]
            )
            / dx**2
            - lambda_nl * u[1:-1, 1:-1, 1:-1] ** 3
        )
    )
    return u_new


# === Peak count ===
def count_peaks(u, threshold_ratio=0.3, sigma=1.0):
    u_smooth = gaussian_filter(u, sigma=sigma)
    threshold = threshold_ratio * np.max(u_smooth)
    binary = u_smooth > threshold
    labeled, num = label(binary)
    return num


# === Main simulation ===
def simulate_D3(steps=500, dt=0.01, lambda_nl=0.57):
    u, dx = initialize_field()
    u_old = u.copy()
    peak_history = []
    snapshots = []

    for _ in tqdm(range(steps), desc="Simulating D=3"):
        u_new = update_field(u, u_old, dx, dt, lambda_nl)
        u_old, u = u, u_new
        peak_count = count_peaks(u)
        peak_history.append(peak_count)
        snapshots.append(u.copy())

    return peak_history, snapshots


# === Structure analysis ===
def analyze_d3_structures(u, threshold_ratio=0.3, sigma=1.0):
    u_smooth = gaussian_filter(u, sigma=sigma)
    threshold = threshold_ratio * np.max(u_smooth)
    binary = u_smooth > threshold
    labeled, num_features = label(binary)
    regions = find_objects(labeled)

    volumes, energies, box_sizes, fractal_dims = [], [], [], []

    for i, slc in enumerate(regions, start=1):
        if slc is None:
            continue
        region = labeled[slc] == i
        volume = np.sum(region)
        peak = np.max(u_smooth[slc][region])
        energy = (peak**2) * volume
        box_size = np.prod([s.stop - s.start for s in slc])

        if volume > 0 and box_size > 0:
            log_box = np.log(box_size)
            if log_box > 1e-8:  # exclude log(1)=0
                fractal = np.log(volume) / log_box
                fractal_dims.append(fractal)

        volumes.append(volume)
        energies.append(energy)
        box_sizes.append(box_size)

    return (
        np.array(volumes),
        np.array(energies),
        np.array(box_sizes),
        np.array(fractal_dims),
    )


# === Execution ===
if __name__ == "__main__":
    peak_series, snapshots = simulate_D3()
    final_field = snapshots[-1]

    volumes, energies, box_sizes, fractals = analyze_d3_structures(final_field)

    # === Visualization ===
    plt.figure(figsize=(14, 4))

    plt.subplot(1, 3, 1)
    plt.hist(volumes, bins=20, color="skyblue", edgecolor="black")
    plt.title("D=3 Structure Volumes")
    plt.xlabel("Volume (voxel count)")
    plt.ylabel("Count")

    plt.subplot(1, 3, 2)
    plt.hist(energies, bins=20, color="gold", edgecolor="black")
    plt.title("D=3 Structure Energies")
    plt.xlabel("Energy (amplitude² × volume)")

    plt.subplot(1, 3, 3)
    plt.hist(fractals, bins=20, color="salmon", edgecolor="black")
    plt.title("D=3 Fractal Dimension Estimates")
    plt.xlabel("Estimated D_f = log(V)/log(BoxSize)")

    plt.tight_layout()
    plt.savefig("D3_structure_analysis.png")
    plt.show()

    # === Statistical output ===
    print(f"\nNumber of structures: {len(volumes)}")
    print(
        f"Volume: mean = {np.mean(volumes):.2f}, max = {np.max(volumes)}, min = {np.min(volumes)}"
    )
    print(f"Energy: mean = {np.mean(energies):.2f}, max = {np.max(energies):.2f}")
    print(
        f"Fractal dimension: mean = {np.mean(fractals):.2f}, "
        f"range = ({np.min(fractals):.2f} to {np.max(fractals):.2f})"
    )
